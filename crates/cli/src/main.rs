...
fn main() {
    ...
    let tone_sounds = helpers::sound_helpers::parse_sounds_from_tone_file(tone_file_text);
    ...
    if output_direction_arg == "--file" {
        ...
        let _ = helpers::sound_helpers::write_audio_to_wav(tone_sounds, &output_filename);
    } else if output_direction_arg == "--sound" {
        helpers::sound_helpers::play_sound_aloud(tone_sounds);
    }
}
